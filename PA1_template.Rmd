...
daterange = c(seq(as.Date(min(dates$date)), as.Date(max(dates$date)), by = 6))
```
...
steps.a.day <- aggregate(steps ~ date, data = activity, 
...
hist(steps.a.day1$steps1000,
            breaks = round(length(dates$date)/3,0)+2
            main = "Total Steps Each Day",
            xlab = "Measurement Date",
            ylab = "Total Steps(000s)",
            xlim = c(0,length(dates$date)), ylim = c(0, 20))
            xaxt = "n")
axis(side = 1, at = steps.a.day1$date, labels = c(daterange))
        

ggplot(steps.a.day1, aes(x = date)) +
      geom_histogram(breaks = round(length(dates$date)/3,0)+2, 
                     fill = "blue", na.rm = TRUE, pad = TRUE) +
      labs(title = "Total Steps Each Day") +
      labs(x = "Date", y = "Total Steps(000s)") +
      scale_x_date(labels = strptime(as.character(steps.a.day1$date), "%Y-%m-%d")) +
      scale_y_continuous(breaks = seq(0, 20, 4)) +
      scale_x_continuous(breaks = c(seq(from = as.Date(min(dates$date)),
                              to = as.Date(max(dates$date)), by = "3 months"") +
            
            geom_rect(aes(xmin = as.Date("2012-10-01", "%Y-%m-%d"),
                          xmax = as.Date("2012-11-30", "%Y-%m-%d"),
                          ymin =0, ymax = 25)) +
            
            
 #          axis.Date(1, as.Date(15103, origin = "1970-01-01")) +
            geom_rect(aes(xmin = as.Date("2012-10-01", "%Y-%m-%d"),
                          xmax = as.Date("2012-11-30", "%Y-%m-%d"),
                          ymin =0, ymax = 25))
```
...
qplot(x = day, y = steps, data = mean.steps.day, group = 7,  
      ...
      main = "Day of Week (Average Steps)")
```
...
#### NOTE:  Results are rounded, no decimals, as partial steps are not counted.
```{r mean_steps_day_interval, include = TRUE}
mean.day.interval <- aggregate(steps ~ day + interval, data = activity,
                              FUN = function(x) mean=round(mean(x), digits = 0), na.rm = TRUE)
...
impute.total.steps <- aggregate(steps ~ date, data = activity.total,
                             FUN = function(x) sum = sum(x))
impute.total.steps$steps1000 <- round(impute.total.steps$steps/1000, digits = 2)
impute.total.steps1000 <- select(impute.total.steps, date, steps1000)
```
